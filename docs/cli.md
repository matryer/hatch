...
Pass `-banner` to prepend a `<!-- Generated by hatch from <source>. Do not
edit… -->` comment to every **file-owned** output (skills, commands, agents,
Cursor `.mdc` rules), naming the `.hatch/` source so a reader knows not to edit
the generated copy. Block-injected files (`CLAUDE.md`, `AGENTS.md`, `.rules`,
`copilot-instructions.md`) already self-identify via their `hatch:begin/end`
markers and are unaffected. The flag is off by default; like `-no-hatch-skill`,
use it consistently across `gen`/`list`/`check`/`clean` (a tree generated with
`-banner` only reads as up to date under `hatch check -banner`).

```
hatch gen -banner                 # add the do-not-edit banner to file-owned outputs
```
