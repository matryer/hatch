...
The `metadata` block is machine-oriented and easy to miss when someone opens a
generated file to edit it. Pass [`-banner`](cli.md#hatch-gen) to additionally
prepend a human-readable comment to each file-owned output:

```markdown
<!-- Generated by hatch from .hatch/_skills/review-pr/SKILL.md. Do not edit directly; edit that file and run `hatch gen`. -->
```

The banner sits just after the frontmatter, names the same `source`, and is
emitted only for file-owned outputs — block-injected files already self-identify
via their markers. It is off by default.
