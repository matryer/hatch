...
Block-injected files announce themselves through these markers. File-owned
files don't, so they can read as ordinary hand-editable files even though the
next `hatch gen` overwrites them. The opt-in [`-banner`](cli.md#hatch-gen) flag
prepends a "generated by hatch — do not edit; edit `<source>`" comment to
file-owned outputs to close that gap.
