...
func TestGenerate_BannerOptIn(t *testing.T) {
	// With Source.Banner set, file-owned outputs (SKILL.md) carry a
	// "generated by hatch" banner naming the source; block-injected files
	// (CLAUDE.md) do not, since they already self-identify via markers.
	// With Banner off, no output carries the banner.
	newSource := func(banner bool) *source.Source {
	...
	const want = "Generated by hatch from .hatch/_skills/review-pr/SKILL.md"

	is := is.New(t)
	arts, err := claude.New().Generate(newSource(true))
	is.NoErr(err)
	byPath := map[string]string{}
	for _, a := range arts {
		byPath[a.Path] = a.Content
	...
	skill := byPath[".claude/skills/review-pr/SKILL.md"]
	is.True(strings.Contains(skill, want))                                     // banner present in the skill...
	is.True(strings.Contains(skill, "---\n\n<!--"))                            // ...placed after the frontmatter block
	is.True(!strings.Contains(byPath["CLAUDE.md"], "Generated by hatch from")) // not in block-injected file

	// Default (Banner off): no banner anywhere.
	offArts, err := claude.New().Generate(newSource(false))
	is.NoErr(err)
	for _, a := range offArts {
		is.True(!strings.Contains(a.Content, "Generated by hatch from"))
	}
}
